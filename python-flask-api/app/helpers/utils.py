...
from app import app, mail, datetime, time
...
class utils:
    ...
    # convinient function sending email. Please make sure that email configuration setings are provided in config.py
    @staticmethod
    ...
    @staticmethod
    def is_unique(model, field, fieldvalue, tablekey=None, recid=None, message=None):
        
        record = model.query.filter(field == fieldvalue).first()
        if not message:
            message = f'{fieldvalue} Already exists'
        if record and (tablekey is None or recid != record[tablekey]):
            return message
        return None
